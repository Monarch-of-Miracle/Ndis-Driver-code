...
CIpFilter* g_Filter = NULL;
NDIS_HANDLE g_FilterHandle = NULL;
...
HANDLE g_hProtectProcessId = 0;
PVOID g_pRegiHandle = NULL;

OB_PREOP_CALLBACK_STATUS MyObjectPreCallback(__in PVOID  RegistrationContext,__in POB_PRE_OPERATION_INFORMATION OperationInformation)
{
	...
	PEPROCESS pProcess = (PEPROCESS)OperationInformation->Object;
	HANDLE hProcessId = PsGetProcessId(pProcess);
	
	if (hProcessId != g_hProtectProcessId)
		return OB_PREOP_SUCCESS;
...
--*/
{
    ...
    do
    {
        ...
        Status = Ndis6FirewallRegisterDevice();
...
--*/
{
    ...
    do
    {
		...
		pFilter->NblPoolHandle = NdisAllocateNetBufferListPool(FilterDriverHandle, &PoolParameters);
...
--*/
{
    ...
    DEBUGP(DL_TRACE, "===>Ndis6Firewall FilterPause: FilterInstance %p\n", FilterModuleContext);
...
--*/
{
    ...
    if (Status != NDIS_STATUS_SUCCESS)
    {
        ...
        //
        PWCHAR              ErrorString = L"Ndis6Firewall";
...
--*/
{
	...
	if (pFilter->NblPoolHandle!= NULL)
	{
		NdisFreeNetBufferListPool(pFilter->NblPoolHandle);
		pFilter->NblPoolHandle = NULL; 
	}
...
--*/
{
	...
	{
		PVOID pCopyData = NULL;
		ULONG BufferLength = 0;

		PMDL pMdl = NET_BUFFER_FIRST_MDL(NET_BUFFER_LIST_FIRST_NB(NetBufferLists));
		...
		NdisFreeMdl(pMdl); 
		NdisFreeMemory(pCopyData, BufferLength, 0); 
...
--*/
{
	...
	//
	if (g_Filter->AnalysisPacket(NetBufferLists, TRUE))
		NdisFSendNetBufferListsComplete(pFilter->FilterHandle, NetBufferLists, NDIS_SEND_COMPLETE_FLAGS_DISPATCH_LEVEL);
	else
...
--*/
{
	...
	//
	if (g_Filter->AnalysisPacket(NetBufferLists, FALSE, pFilter))
		NdisFReturnNetBufferLists(pFilter->FilterHandle, NetBufferLists, NDIS_RETURN_FLAGS_DISPATCH_LEVEL);
	else
		NdisFIndicateReceiveNetBufferLists(
